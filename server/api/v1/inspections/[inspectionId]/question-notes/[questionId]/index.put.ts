...
import { saveInspectionQuestionNote } from "../../../../../../utils/services/save-inspection-question-note";
import {
...
import type { PutInspectionQuestionNoteResult } from "../../../../../../../shared/contracts/inspections";

export default defineEventHandler(
  async (
    event,
  ): Promise<ApiSuccessResponseDto<PutInspectionQuestionNoteResult>> => {
    ...
    const parseResult =
      PutInspectionQuestionNoteCommandSchema.safeParse(rawBody);
    ...
    // ── Domain operation ───────────────────────────────────────────────────
    let result: PutInspectionQuestionNoteResult;
    try {
      result = await saveInspectionQuestionNote(
        event,
        userId,
        inspectionId,
        questionId,
      ...
      console.error(
        "[PUT /api/v1/inspections/:inspectionId/question-notes/:questionId] unexpected error",
        {
          requestId,
          userId,
          inspectionId,
